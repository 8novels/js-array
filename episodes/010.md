...
あ「これでいいのか」
イ「あれい、また解説をお願い！」
あ「ああん？　面倒くせえなあ……だから、`map()` メソッドですべての記事にアクセスして`title`プロパティを取得するだろ、そしてさらに各記事の`tags`プロパティに`map()` メソッドでアクセスして、すべてのアイテムの`name`プロパティの値を取得してから`join()`メソッドで連結が'JavaScript'のタグが1個でもあれば`true`を返してだな、それを`filter()`メソッドで抽出してるんだよ」
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDIyMzgwODY4LDE2NTQ0NDI1NzksLTg3Nz
