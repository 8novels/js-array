...
あ「これでいいのか」
イ「あれい、また解説をお願い！」
あ「ああん？　面倒くせえなあ……だから、`map()` メソッドですべての記事にアクセスして`title`プロパティを取得するだろ、そしてさらに各記事の`tags`プロパティ内のすべての`name`プロパティに`map()` メソッドでアクセスして、それを`join()`メソッドで連結してるんだよ」
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg0OTk1NzY5OSwxNjU0NDQyNTc5LC04Nz
