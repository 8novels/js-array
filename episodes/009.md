...
あ「これでいいのかよ」
イ「あれい、また解説をお願い！」
あ「ああん？　面倒くせえなあ……だから、まずは本文中に'JavaScript'という文字列を含む記事だけを抽出してだな、記事アイテムの`tags`プロパティを`some()` メソッドで検索して、`name`が'JavaScript'のタグが1個でもあれば`true`を返してだな、
次に`every()` メソッドで`likes_count`の値が10以上のものだけを抽出してるんだよ」
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1NDk1MjUyNTIsLTQyODAzNjIyMCw2Nz
kzNzE2NjMsLTExMzAxNzI4NDYsNTcxMzQ1NTM4XX0=
-->