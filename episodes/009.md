...
`every()` メソッドは、`some()` メソッドと違い、配列のすべての要素に対してテスト関数を実行し、すべての要素が条件に合致した場合のみ`true`を、そうでなければ`false`を返します。
よく似ており、テスト関数に渡される引数も同じです。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjg4MzY5MTAwLDU3MTM0NTUzOF19
-->